...
if (isset($_GET['id']) && $_SERVER["REQUEST_METHOD"] != "POST") {
    // Récupérer l'identifiant du stage à modifier depuis l'URL
    $offreId = $_GET['id'];

    // Récupérer les détails du stage à partir de la base de données
    $offreController = new Offres_stageC();
    $stagedetail = $offreController->getOffreById($offreId);

    // Vérifier si les détails du stage ont été récupérés avec succès
    if ($stagedetail) {
        // Pré-remplir le formulaire avec les détails du stage
        $titre = $stagedetail['titre'];
        $entreprise = $stagedetail['entreprise'];
        $type_stage = $stagedetail['type_stage'];
		$duree = $stagedetail['dureee'];
        $domaine = $stagedetail['domaine'];
        $nombre_place = $stagedetail['nombre_place'];
        $description = $stagedetail['description'];
    } else {
        echo "Erreur: Impossible de récupérer les détails du stage.";
        exit;
    }
}
...
if (isset($_POST['Modifier']) && $valide) {
    ...
    $offre = new Offres_stage(null, $titre, $entreprise, $type_stage,$duree, $domaine, $nombre_place, $description);
    
    // Ajout du stage à la base de données
    $offreController = new Offres_stageC();
    $offreController->updateOffrestage($offre, $_GET['id']);
...
<body>
	...
	<div class="main-container">
        ...
        <table>
            ...
            <tr>
                <td><label for="type_stage">Type de stage :</label></td>
                <td>
                    <select id="type_stage" name="type_stage" required>
                        <option value="CDI" <?php echo (isset($type_stage) && $type_stage == 'CDI') ? 'selected' : ''; ?>>CDI</option>
                        <option value="CDD" <?php echo (isset($type_stage) && $type_stage == 'CDD') ? 'selected' : ''; ?>>CDD</option>
                        <option value="CTT" <?php echo (isset($type_stage) && $type_stage == 'CTT') ? 'selected' : ''; ?>>CTT</option>
                    </select>
                </td>
            </tr>
            <tr>
                <td><label for="domaine">duree :</label></td>
                <td><input type="text" id="duree" name="duree" required value="<?php echo isset($duree) ? $duree : ''; ?>"></td>
            </tr>
			<tr>
            ...
            <tr>
                ...
                <td><a href="listStage.php">Annuler</a></td>
            </tr>
        </table>
    </form>
