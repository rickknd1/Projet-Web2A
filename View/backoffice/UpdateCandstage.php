...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $candidatureController = new candidaturestageC();
    $candidatureController->updateCandidaturestageStatut($candidature_id, $new_statut);
	$offre_id = $candidatureController->getOffreIdByCandidaturestageId($candidature_id);
    // Utilisation de la fonction pour envoyer un e-mail
	$destinataire = 'hadjsalemadem47@gmail.com';
    $sujet = 'Affectation demploi';
	$header="From EspritCareersConnect ";
	if ($new_statut == 'Acceptée') {
        $message = 'Felicitation vous avez été accepté pour une offre d emploie , verifiez sur le site pour plus dinformation';
		$offrestage = new Offres_stageC();
		$offrestage->decrementPlaces($offre_id);
    } else if ($new_statut == 'Refusée') {
        $message = 'Votre candidature a été refusée pour une offre d emploie , verifiez sur le site pour plus dinformation';
    }
    if(mail($destinataire,$sujet,$message,$header)){
		echo" votre mail a ete email envoyé";
		echo "<script>alert('votre mail a ete envoyé');</script>";
	}
	else{
		echo"email failed";
		echo "<script>alert('email failed');</script>";
	}
    // Redirection vers la liste des candidatures après la mise à jour
...
<body>
	...
	<div class="main-container">
					...
					<div class="col-md-6 col-sm-12">
						...
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item">
									<a href="listOfrstage.php">Offre de stage</a>
								</li>
                                <li class="breadcrumb-item">
									<a href="listCandstage.php">Candidatures</a>
								</li>
        ...
        <h2>Affectation de Candidature</h2>
        <form action="UpdateCandstage.php" method="POST">
            <input type="hidden" name="candidature_id" value="<?= $_GET['candidaturestage_id']; ?>">
