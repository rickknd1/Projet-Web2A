...
<section id="ajout-candidature" class="candidature-section">
    <div class="container">
        ...
        <br>
        <a href="offrestage.php" class="btn btn-secondary">Retour à la liste des stages</a>
    </div>
</section>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['id'])) {
    ...
    $candidature = new Candidaturestage(null,$_GET['id'], $_SESSION['id'],  $date_candidature, $lettre_motivation, $cv_path, $statut);

    // Ajoute la candidature à la base de données
    $candController = new CandidaturestageC();
    $candController->addcandidaturestage($candidature);

    // Redirection vers offrestage.php dans le dossier backoffice
    header('Location: /2A34/Main-main/View/frontoffice/offrestage.php');
    exit; // Assurez-vous d'utiliser exit() après la redirection
   
    
} else {
    // Si l'ID de l'offre d'stage n'est pas présent dans l'URL, affiche un message d'erreur ou redirige l'utilisateur
    echo "Erreur: ID de l'offre de stage manquant dans l'URL";
}
?>
